/**
 * DeviceDetector - Utility for detecting device type, browser, and operating system
 * 
 * Provides methods to detect device type (phone/tablet/desktop), browser type, and operating system
 * from user agent strings. Primary purpose is device detection for responsive layouts.
 * 
 * @example
 * import DeviceDetector from './src/utils/device/DeviceDetector.js';
 * 
 * const deviceType = DeviceDetector.getDeviceType();
 * const browser = DeviceDetector.getBrowser();
 * const os = DeviceDetector.getOS();
 * console.log(`Running on ${deviceType} device: ${browser} on ${os}`);
 */
export default class DeviceDetector {
    ...
    static _hasIPadTraits(userAgent = '') {
        const nav = DeviceDetector._getNavigator();
     ...
     */
    static isiPadOS(userAgent = DeviceDetector._getNavigator().userAgent || '') {
        return DeviceDetector._hasIPadTraits(userAgent);
    }
    ...
    static getBrowser(userAgent = navigator.userAgent) {
        ...
        if (DeviceDetector.isiPadOS(ua) && /Safari/.test(ua) && !/Chrome/.test(ua)) {
            return 'Safari iOS';
        }
    ...
    static getBrowserAndOS(userAgent = navigator.userAgent) {
        return {
            browser: DeviceDetector.getBrowser(userAgent),
    ...
    static isIOS(userAgent = navigator.userAgent) {
        return /iPhone|iPad|iPod/.test(userAgent) || DeviceDetector.isiPadOS(userAgent);
    }
    ...
    static isMobile(userAgent = navigator.userAgent) {
        return DeviceDetector.isIOS(userAgent) || DeviceDetector.isAndroid(userAgent);
    }
    
    /**
     * Get the current device orientation
     * @deprecated Use ViewportUtils.getOrientation(scene) for Phaser-based code instead.
     * This method uses window dimensions which may not account for mobile browser UI overlays.
     ...
     */
    static getBrowserOSAndOrientation(userAgent = navigator.userAgent, scene = null) {
        let orientation = 'Portrait';
        if (scene) {
            // Use scene dimensions for orientation
            const sceneWidth = scene.sys?.game?.scale?.width || window.innerWidth;
            const sceneHeight = scene.sys?.game?.scale?.height || window.innerHeight;
            orientation = sceneHeight > sceneWidth ? 'Portrait' : 'Landscape';
        } else {
    ...
    static isTablet(width = window.innerWidth, height = window.innerHeight, userAgent = navigator.userAgent) {
        return DeviceDetector.getDeviceType(width, height, userAgent) === 'tablet';
    }
    ...
    static isPhone(width = window.innerWidth, height = window.innerHeight, userAgent = navigator.userAgent) {
        return DeviceDetector.getDeviceType(width, height, userAgent) === 'phone';
    }
    
    /**
     * Check if device is mobile (phone or tablet) - compatibility method for DeviceUtils migration
     * @param {number} [width] - Screen width in pixels (defaults to window.innerWidth)
     * @param {number} [height] - Screen height in pixels (defaults to window.innerHeight)
     * @param {string} [userAgent] - Optional user agent string (defaults to navigator.userAgent)
     * @returns {boolean} True if device is phone or tablet
     */
    static isMobileDevice(width = window.innerWidth, height = window.innerHeight, userAgent = navigator.userAgent) {
        const deviceType = DeviceDetector.getDeviceType(width, height, userAgent);
        return deviceType === 'phone' || deviceType === 'tablet';
    }
    
    /**
     * Check if device is desktop - compatibility method for DeviceUtils migration
     * @param {number} [width] - Screen width in pixels (defaults to window.innerWidth)
     * @param {number} [height] - Screen height in pixels (defaults to window.innerHeight)
     * @param {string} [userAgent] - Optional user agent string (defaults to navigator.userAgent)
     * @returns {boolean} True if device is desktop
     */
    static isDesktopDevice(width = window.innerWidth, height = window.innerHeight, userAgent = navigator.userAgent) {
        return DeviceDetector.getDeviceType(width, height, userAgent) === 'desktop';
    }
    ...
    static getDeviceType(width = window.innerWidth, height = window.innerHeight, userAgent = navigator.userAgent) {
        ...
        const os = DeviceDetector.getOS(ua);
     ...
     */
    static getDeviceInfo(width = window.innerWidth, height = window.innerHeight, userAgent = navigator.userAgent, scene = null) {
        const deviceType = DeviceDetector.getDeviceType(width, height, userAgent);
        const os = DeviceDetector.getOS(userAgent);
        ...
        if (scene) {
            ...
            // ViewportUtils.getOrientation() uses ViewportHelper which is more accurate, but for this method
            // we use scene dimensions directly to avoid import cycles
            const sceneWidth = scene.sys?.game?.scale?.width || width;
            const sceneHeight = scene.sys?.game?.scale?.height || height;
            const orientationLower = sceneHeight > sceneWidth ? 'portrait' : (sceneWidth > sceneHeight ? 'landscape' : 'portrait');
        ...
        return {
            ...
            orientation: orientation,
            isMobile: DeviceDetector.isMobile(userAgent), // OS-based (iOS/Android)
            isMobileDevice: DeviceDetector.isMobileDevice(width, height, userAgent), // Device-type-based (phone/tablet)
            isDesktopDevice: DeviceDetector.isDesktopDevice(width, height, userAgent), // Device-type-based
            isTablet: DeviceDetector.isTablet(width, height, userAgent),
            isPhone: DeviceDetector.isPhone(width, height, userAgent),
            isIOS: DeviceDetector.isIOS(userAgent),
            isAndroid: DeviceDetector.isAndroid(userAgent),
            width: width,
            height: height
        };
    }
