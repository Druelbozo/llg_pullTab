/**
 * Portrait Mobile Layout for Mobile Portrait Orientation
...
export function calculatePortraitMobileLayout(responsiveUI) {
    ...
    if (!containers || !Array.isArray(containers) || containers.length === 0) {
        error(`PortraitMobileLayout: controlBar.layoutGroups is required in layout JSON config but was not found.`);
        return {
            cardContainerX: 0,
            cardContainerY: 0,
    ...
    const areaPercent = cardContainerConfig.areaPercent ?? cardContainerConfig.heightPercent ?? 1.00;
    ...
    if (controlBarBackgroundTop !== undefined && controlBarBackgroundTop !== null && 
        ...
        warn(`[PortraitMobileLayout] Invalid controlBarBackgroundTop (${controlBarBackgroundTop}), using fallback: ${fallbackControlBarTop}`, 'layout');
    }
    ...
    if (cardContainerY < 0 || cardContainerY > height || isNaN(cardContainerY)) {
        ...
        warn(`[PortraitMobileLayout] cardContainerY out of bounds, using emergency fallback: ${cardContainerY}`, 'layout');
    }
