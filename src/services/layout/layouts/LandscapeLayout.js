 ...
 * Optimized for desktop devices in landscape orientation
 */
...
import { calculateCardContainerSize } from '../../../utils/layout/CardContainerUtils.js';

/** @typedef {import('../core/LayoutTypes.js').LayoutPositions} LayoutPositions */

/**
 * Calculate landscape layout positions for landscape orientation
 * 
 * @param {LayoutManager} responsiveUI - The LayoutManager instance
 * @returns {LayoutPositions} Layout positions object
 */
export function calculateLandscapeLayout(responsiveUI) {
    ...
    const { uiConfig, baseUIConfig } = getLayoutConfigs(responsiveUI, 'landscape');
    ...
    // Get containers array from layout config (controlBar.layoutGroups)
    const containers = uiConfig?.controlBar?.layoutGroups || baseUIConfig?.controlBar?.layoutGroups;
    if (!containers || !Array.isArray(containers) || containers.length === 0) {
        error(`LandscapeLayout: controlBar.layoutGroups is required in layout JSON config but was not found.`);
        return {
            cardContainerX: 0,
            cardContainerY: 0,
    ...
    // Calculate container dimensions using universal function
    ...
    // For now, use natural widths (containerWidths will be calculated by ControlBarManager)
    const containerWidths = {}; // Will be populated by ControlBarManager
    ...
    // ============================================================================
    // Get card container configuration
    ...
    const cardContainerX = centerX; // Center horizontally
    ...
    if (controlBarBackgroundTop !== undefined && controlBarBackgroundTop !== null && 
        ...
        warn(`[LandscapeLayout] Invalid controlBarBackgroundTop (${controlBarBackgroundTop}), using fallback: ${fallbackControlBarTop}`, 'layout');
    }
    ...
    if (cardContainerY < 0 || cardContainerY > height || isNaN(cardContainerY)) {
        ...
        warn(`[LandscapeLayout] cardContainerY out of bounds, using emergency fallback: ${cardContainerY}`, 'layout');
    }
